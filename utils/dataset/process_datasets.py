...
def main():
    ...
    if not args.skip_mix:
        mix_cmd = f"python utils/dataset/mix_commonsense_datasets.py"
    ...
    if not args.skip_clean:
        clean_cmd = "python utils/dataset/final_data_cleaner.py"
    ...
    if success:
        print("🎉 数据集处理流程完成！")
        print("\n📁 输出文件:")
        print(f"   - {args.output_dir}/cs_all.jsonl (原始合并数据)")
        print(f"   - {args.output_dir}/cs_all_final.jsonl (清理后数据)")
        print(f"   - {args.output_dir}/dataset_stats.json (统计信息)")
    else:
        print("❌ 处理流程未完全成功")
    print("=" * 60)
